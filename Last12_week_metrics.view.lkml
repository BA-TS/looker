view: last12_week_metrics {
   ...
   derived_table: {
     sql: with sub1 as (SELECT distinct date, minTime, platform, deviceCategory, session_id, page_location, screen_name,
aw.item_id,
...
purchase as (select distinct session_id as purchase_session, minTime as purchase_time, item_id as purchase_id, sum(net) as net, sum(Quantity) as quantity
from sub1 where event_name in ("purchase", "Purchase")
group by all),

 search as (select distinct session_id as search_session
...
select distinct row_number() over () as P_K, sub1.date as date, platform, deviceCategory,sub1.session_id as all_sessions,
count(distinct page_location) over (partition by sub1.session_id) pages_in_session,
page_location, screen_name, screen_type,
landingPage,landingScreen, LandingScreenType,
exitPage,exitScreen, exitScreenType,
