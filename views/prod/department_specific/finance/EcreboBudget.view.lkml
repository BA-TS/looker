...
round((safe_divide(Trade_5__Discount, count(distinct dd.fullDate))*-1),2) as daily_budget
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Leyland, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Screed, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Other__Multibuy_, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Free_product_offers_, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Vouchers_in_store, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(cast(Vouchers_on_app as int64), count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(App_download_discount, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Managed_services, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Staff_Discount, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(BLC_Discount, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(cast(CRM as int64), count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(cast(CRM___Loyalty as int64), count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(cast(Other as int64), count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Affiliates, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Balance, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
...
from (
SELECT distinct be.FiscalYearWeek as FYW,
round((safe_divide(Total, count(distinct dd.fullDate))*-1),2) as daily_budget
FROM `toolstation-data-storage.ecrebo.2024_budget_ecrebo` as be
inner join `toolstation-data-storage.ts_finance.dim_date` as dd on be.FiscalYearWeek = dd.fiscalYearWeek
