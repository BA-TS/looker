...
# include: "/views/prod/date/fixed_PoP.view"
    ...
    dimension_group: date {
      ...
      can_filter: no
      hidden:  yes # use sessions_date (partition_date)
      allow_fill: no
    }
    ...
    dimension: pivot_dimension {
      ...
      sql:
           {% if select_date_range._is_filtered %}
              CASE
                WHEN {% condition select_date_range %}  ${__target_date__} {% endcondition %}
                THEN "This {% parameter select_comparison_period %}"
                WHEN ${__target_date__} >= ${period_2_start} and ${__target_date__} < ${period_2_end}
                THEN "Last {% parameter select_comparison_period %}"
                WHEN ${__target_date__}  >= ${period_3_start} and ${__target_date__} < ${period_3_end}
    ...
    filter: flexi_pop {
      ...
      {% if select_date_range._in_query %}
        ${__target_date__} >= DATE(TIMESTAMP_SUB({% date_start select_date_range %} , INTERVAL 0 DAY)) AND ${__target_date__} < DATE(TIMESTAMP_SUB({% date_end select_date_range %} , INTERVAL 0 DAY))
         {% if select_date_range._is_filtered and (select_number_of_periods._in_query or select_comparison_period._in_query) %}
           {% if select_number_of_periods._parameter_value == "2" %}
               or ${__target_date__} >= DATE(${period_2_start}) AND ${__target_date__} < DATE(${period_2_end})
             {% elsif select_number_of_periods._parameter_value == "3" %}
               or ${__target_date__} >= DATE(${period_2_start}) AND ${__target_date__} < DATE(${period_2_end})
               or ${__target_date__} >= DATE(${period_3_start}) AND ${__target_date__} < DATE(${period_3_end})
           {% endif %}
         {% endif %}
      {% else %}
        TRUE
    ...
    dimension: day_in_period {
      ...
      sql:
          {% if select_number_of_periods._is_filtered or select_comparison_period._in_query %}
            CASE
              WHEN {% condition select_date_range %} ${__target_date__} {% endcondition %}
              THEN TIMESTAMP_DIFF(${__target_date__},{% date_start select_date_range %},DAY)+1
              WHEN ${__target_date__} >= ${period_2_start} and ${__target_date__} < ${period_2_end}
              THEN TIMESTAMP_DIFF(${__target_date__}, ${period_2_start},DAY)+1
              WHEN ${__target_date__} >= ${period_3_start} and ${__target_date__} < ${period_3_end}
              THEN TIMESTAMP_DIFF(${__target_date__}, ${period_3_start},DAY)+1
            END
          {% endif %};;
      hidden: yes
    }
    ...
    dimension: order_for_period {
      ...
      {% if select_number_of_periods._is_filtered or select_comparison_period._in_query and select_date_range._in_query %}
        CASE
          WHEN {% condition select_date_range %} ${__target_date__} /*findme6*/{% endcondition %}
          THEN 1
          WHEN ${__target_date__} >= ${period_2_start} and ${__target_date__} < ${period_2_end}
          THEN 2
          WHEN ${__target_date__} >= ${period_3_start} and ${__target_date__} < ${period_3_end}
    ...
    dimension: week_to_date {
      type: yesno
      sql:EXTRACT(DAYOFWEEK FROM ${__target_date__}) <= EXTRACT(DAYOFWEEK FROM ${__current_date__})
    ...
    dimension: week_to_date_LW {
      type: yesno
      sql:EXTRACT(DAYOFWEEK FROM ${__target_date__}) <= EXTRACT(DAYOFWEEK FROM ${__current_date__} - ${__length_of_week__})
    ...
    dimension: week_to_date_2LW {
      type: yesno
      sql:EXTRACT(DAYOFWEEK FROM ${__target_date__}) <= EXTRACT(DAYOFWEEK FROM ${__current_date__} - (2*${__length_of_week__}))
    ...
    dimension: month_to_date {
      type: yesno
      sql:${__target_date__} > ${__current_date__} - EXTRACT(DAY FROM ${__current_date__}) AND ${__target_date__} <= ${__current_date__};;
      hidden: yes
    }
