...
# ╚═════╝░╚═╝░░╚═╝░░░╚═╝░░░╚══════╝
    ...
    dimension_group: date {
      ...
      {% if pivot_dimension._in_query %}
        ...
        ${base_date_raw}
        {% endif %}
      {% elsif select_fixed_range._parameter_value == "YLM" %} ${base_date_raw} between timestamp_trunc(${__current_date__}, year) and  timestamp(last_day(date_sub(${__current_date__}, INTERVAL 1 month), month))
      {% else %}
        ${base_date_raw}
      {% endif %};;
      timeframes: [date]
