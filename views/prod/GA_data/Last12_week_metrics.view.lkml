view: last12_week_metrics {
 ...
 with sub1 as (
  --SELECT distinct date, minTime, platform, deviceCategory, session_id,
...
--       and _TABLE_Suffix between format_date("%Y%m%d", date_sub(date("2025-02-28"), interval 10 day)) and format_date("%Y%m%d",date("2025-02-28"))
--       group by all
-- union distinct
select distinct
...
case when event_name in ("add_to_cart") and platform in ("Web") then regexp_extract(label_2, "^.*\\-(.*)$") else label_2 end as label_2,
    ...
    regexp_extract(fu, ".*\\:(.*)") as filter_label,
      ...
      --row_number () over (partition by session_id order by minTime asc) as landingP,
      --row_number () over (partition by session_id order by minTime desc) as exitP
      FROM `toolstation-data-storage.Digital_reporting.GAServerSide_DigitalTransactions_*` aw left join unnest(transactions) as transactions
      left join unnest(SPLIT(filters_used, ",")) as fu WITH OFFSET as test2
      where ((aw.item_id = transactions.productCode) or (aw.item_id is not null and transactions.productCode is null) or (aw.item_id is null and transactions.productCode is null))

      and _TABLE_Suffix between format_date("%Y%m%d",date("2025-03-01")) and format_date("%Y%m%d",current_date())
      group by all)
),
...
case when event_name in ("add_to_cart") and platform in ("Web") then regexp_extract(label_2, "^.*\\-(.*)$") else label_2 end as label_2,
    ...
    regexp_extract(fu, ".*\\:(.*)") as filter_label,
      ...
      and _TABLE_Suffix between format_date("%Y%m%d", date_sub(date("2025-02-28"), interval 12 week)) and format_date("%Y%m%d",date("2025-02-28"))
      group by all
-- union distinct
-- select distinct
...
--       and _TABLE_Suffix between format_date("%Y%m%d",date("2025-03-01")) and format_date("%Y%m%d",date("2025-03-10"))
--       group by all)
),
