- dashboard: branch_performance_drr
  title: Branch Performance (DRR)
  layout: newspaper
      ...
      <div style="padding: 20px 0 20px 0; border-radius: 5px; background: #ffe200; height: 80px;">
         ...
         <div style="background: #004f9f; height: 40px; width:100%">
            ...
            <nav style="font-size: 18px;">
               ...
               <a style="color: #ffffff; padding: 0 20px ; float: right; line-height: 40px; font-weight: regular" href="https://tpdev.cloud.looker.com/embed/dashboards-next/151" target="_blank" fullscreen="yes">View Full Screen</a>
              </span>
               <a style="color: #efefef; padding: 0 20px; float: right; line-height: 40px; font-weight: bold; text-decoration: none;"><span style="color: #ffffff;">Daily Sales Report - Branch (Performance)</span></a>
            </nav>
         </div>
  ...
  - name: " (2)"
    ...
    width: 24
    height: 2
  - title: Branch Performance (By Store)
    name: Branch Performance (By Store)
    model: ts_sales
    explore: base
    type: looker_grid
    fields: [transactions.total_net_sales, site_budget.site_net_sales_budget, base.date_year,
      transactions.lfl_net_sales, sites.region_manager, sites.region_name, transactions.site_uid,
      sites.site_name]
    pivots: [base.date_year]
    filters:
      base.select_fixed_range: PD
      base.select_comparison_period: Year
    sorts: [base.date_year desc, transactions.total_net_sales desc 0]
    limit: 500
    column_limit: 50
    total: true
